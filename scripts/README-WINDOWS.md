...
适用包名：`ace-mcp-v4.10.4-win-x64.zip`。

发布状态：v4.10.4 自包含 ZIP 尚待回到 Windows 主机补充构建与验证；该产物必须在 Windows x64 + Node.js 22 环境生成并通过原生依赖、CLI、doctor 和 Web smoke，不能用 macOS 构建结果替代。
...
1. 解压 `ace-mcp-v4.10.4-win-x64.zip`。
2. 双击 `start-web.cmd`。
