...
适用包名：`ace-mcp-v4.10.10-win-x64.zip`。

发布状态：v4.10.10 自包含 ZIP 尚待回到 Windows 主机补充构建与验证；该产物必须在 Windows x64 + Node.js 22 环境生成并通过原生依赖、CLI、doctor 和 Web smoke，不能用 macOS 构建结果替代。
...
## v4.10.10 摘要关键符号与强制刷新

- 摘要生成使用真实索引定义填充模块关键符号，不再使用通配字符串模拟符号枚举。
- `generate_summary`、Web 摘要接口和 `reindex-projects.mjs --summary --force-summary` 可强制重生成全部模块与 architecture。
...
1. 解压 `ace-mcp-v4.10.10-win-x64.zip`。
2. 双击 `start-web.cmd`。
