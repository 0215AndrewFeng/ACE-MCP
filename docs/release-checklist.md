...
```bash
tar -tf ace-mcp-4.9.5.tgz | rg "package/(dist/index.js|scripts/install-macos.sh|scripts/verify-release-assets.mjs|scripts/check-secrets.mjs|scripts/publish-gitee-release.mjs|scripts/start-web.cmd|scripts/README-WINDOWS.md|scripts/smoke-release.mjs|scripts/benchmark-search.mjs|scripts/reindex-projects.mjs)"
unzip -l release/ace-mcp-v4.9.5-win-x64.zip | rg "dist/index.js|install.ps1|start-web.cmd|README-WINDOWS.md|benchmark-search.mjs|check-secrets.mjs|publish-gitee-release.mjs|reindex-projects.mjs"
```
...
```bash
git add .
git commit -m "feat: release v4.9.5 search result actions"
git tag -a v4.9.5 -m "v4.9.5"
