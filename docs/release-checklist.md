...
v4.10.7 的 source、annotated tag、远端推送和 Gitee Release 资产状态必须以 Git/Gitee 实际结果为准。Windows ZIP 和 `release:smoke` 仍待 Windows x64 + Node.js 22 环境完成，不得把当前 Darwin 验证记录成跨平台门禁通过。
...
截至本清单更新时尚未补齐 v4.10.7 Windows ZIP；必须稍后在 Windows x64 + Node.js 22 主机执行构建、原生绑定加载和 smoke 验证，Darwin 主机结果不能替代该门禁。
...
```bash
tar -tf ace-mcp-4.10.7.tgz > /tmp/ace-mcp-tgz-files.txt
...
unzip -Z1 release/ace-mcp-v4.10.7-win-x64.zip > /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/dist/index.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/dist/core/storage/sqliteIndexWorker.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/dist/web/static/js/app.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/dist/web/static/css/main.css" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/runtime/node.exe" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/node_modules/better-sqlite3/build/Release/better_sqlite3.node" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/ace-mcp.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/ace-mcp-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/start-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/doctor.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/README-WINDOWS.md" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.7-win-x64/scripts/reindex-projects.mjs" /tmp/ace-mcp-win-files.txt
```
...
git diff --cached
git commit -m "fix: release v4.10.7 Codex sandbox initialization"
git tag -a v4.10.7 -m "v4.10.7"
git push origin master
git push origin v4.10.7
```

只有在 staged diff 的文件范围和内容均确认属于 v4.10.7、且对应平台门禁完成后，才执行 commit/tag/push。
