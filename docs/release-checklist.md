...
截至本清单更新时尚未补齐 v4.10.4 Windows ZIP；必须稍后在 Windows x64 + Node.js 22 主机执行构建、原生绑定加载和 smoke 验证，Darwin 主机结果不能替代该门禁。
...
```bash
tar -tf ace-mcp-4.10.4.tgz > /tmp/ace-mcp-tgz-files.txt
...
unzip -Z1 release/ace-mcp-v4.10.4-win-x64.zip > /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/dist/index.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/dist/core/storage/sqliteIndexWorker.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/dist/web/static/js/app.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/dist/web/static/css/main.css" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/runtime/node.exe" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/node_modules/better-sqlite3/build/Release/better_sqlite3.node" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/ace-mcp.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/ace-mcp-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/start-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/doctor.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/README-WINDOWS.md" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.4-win-x64/scripts/reindex-projects.mjs" /tmp/ace-mcp-win-files.txt
```
...
git diff --cached
git commit -m "fix: release v4.10.4 bounded logging"
git tag -a v4.10.4 -m "v4.10.4"
git push origin master
git push origin v4.10.4
```

只有在 staged diff 的文件范围和内容均确认属于 v4.10.4、且 Phase 6 门禁完成后，才执行 commit/tag/push。

9. 使用 Gitee OpenAPI 创建/更新 Release、替换同名 tgz/Windows zip 附件，并自动验证真实下载链路：

```bash
GITEE_TOKEN=<your-token> npm run release:publish -- --version 4.10.4
```

`release:publish` 默认会在上传后运行 `release:verify-assets`。如需单独复验：

```bash
npm run release:verify-assets -- --version 4.10.4
```
