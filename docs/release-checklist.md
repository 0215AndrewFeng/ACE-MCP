...
截至本清单更新时尚未补齐 v4.10.9 Windows ZIP；必须稍后在 Windows x64 + Node.js 22 主机执行构建、原生绑定加载和 smoke 验证，Darwin 主机结果不能替代该门禁。
...
```bash
tar -tf ace-mcp-4.10.9.tgz > /tmp/ace-mcp-tgz-files.txt
...
unzip -Z1 release/ace-mcp-v4.10.9-win-x64.zip > /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/dist/index.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/dist/core/storage/sqliteIndexWorker.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/dist/web/static/js/app.js" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/dist/web/static/css/main.css" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/runtime/node.exe" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/node_modules/better-sqlite3/build/Release/better_sqlite3.node" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/ace-mcp.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/ace-mcp-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/start-web.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/doctor.cmd" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/README-WINDOWS.md" /tmp/ace-mcp-win-files.txt
rg -Fx "ace-mcp-v4.10.9-win-x64/scripts/reindex-projects.mjs" /tmp/ace-mcp-win-files.txt
```
...
git diff --cached
git commit -m "feat: release v4.10.9 summary-assisted project routing"
git tag -a v4.10.9 -m "v4.10.9"
git push origin master
git push origin v4.10.9
git push github master
git push github v4.10.9
```

只有在 staged diff 的文件范围和内容均确认属于 v4.10.9、且对应平台门禁完成后，才执行 commit/tag/push。推送后必须分别核对 `origin/master`、`github/master` 和两端 tag peeled commit。
...
GitHub 使用 `gh release create v4.10.9 ace-mcp-4.10.9.tgz --repo 0215AndrewFeng/ACE-MCP --title "v4.10.9" --notes-file <release-notes>`；Windows ZIP 未通过平台门禁时，两端都不得上传或声称已验证该资产。
