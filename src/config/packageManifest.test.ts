...
test("Windows README documents zip installation and MCP client command paths", () => {
  ...
  assert.match(windowsReadme, /ace-mcp-v4\.7\.6-win-x64\.zip/);
...
test("release checklist records the v4.7.6 verification gates", () => {
  const checklist = readFileSync(path.join(rootDir, "docs/release-checklist.md"), "utf8");

  assert.match(checklist, /v4\.7\.6/);
  ...
  assert.match(checklist, /git tag -a v4\.7\.6/);
});
