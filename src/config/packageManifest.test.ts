...
test("package manifest is ready for npm and tgz global installation", () => {
  ...
  assert.equal(pkg.version, "4.10.9");
  assert.equal(lock.version, pkg.version);
  assert.equal(lock.packages?.[""]?.version, pkg.version);
  assert.match(versionTs, /APP_VERSION\s*=\s*"4\.10\.9"/);
...
test("macOS quick install script and docs are packaged for one-command setup", () => {
  ...
  assert.match(installScript, /ACE_MCP_VERSION="\$\{ACE_MCP_VERSION:-4\.10\.9\}"/);
  assert.match(readme, /bash -c "\$\(curl -fsSL https:\/\/gitee\.com\/AndrewFengCode\/ace-mcp\/raw\/v4\.10\.9\/scripts\/install-macos\.sh\)"/);
  ...
  assert.match(readme, /ACE_MCP_VERSION=4\.10\.9/);
...
test("release asset verifier documents Gitee tag and downloadable artifacts", () => {
  ...
  assert.match(readme, /npm run release:verify-assets -- --version 4\.10\.9/);
  assert.match(readme, /raw\/v4\.10\.9\/scripts\/install-macos\.sh/);
  assert.doesNotMatch(readme, /raw\/master\/scripts\/install-macos\.sh/);

  assert.match(checklist, /npm run release:verify-assets -- --version 4\.10\.9/);
  assert.match(checklist, /ace-mcp-4\.10\.9\.tgz/);
  assert.match(checklist, /ace-mcp-v4\.10\.9-win-x64\.zip/);
  assert.match(checklist, /tar -tf ace-mcp-4\.10\.9\.tgz > \/tmp\/ace-mcp-tgz-files\.txt/);
  ...
  assert.match(checklist, /unzip -Z1 release\/ace-mcp-v4\.10\.9-win-x64\.zip > \/tmp\/ace-mcp-win-files\.txt/);
  assert.match(checklist, /rg -Fx "ace-mcp-v4\.10\.9-win-x64\/dist\/web\/static\/js\/app\.js" \/tmp\/ace-mcp-win-files\.txt/);
  assert.match(checklist, /rg -Fx "ace-mcp-v4\.10\.9-win-x64\/dist\/web\/static\/css\/main\.css" \/tmp\/ace-mcp-win-files\.txt/);
});
...
test("Gitee release publisher documents token-based automated release upload", () => {
  ...
  assert.match(readme, /npm run release:publish -- --version 4\.10\.9/);
  assert.match(readme, /GITEE_TOKEN/);
  assert.match(readme, /release:verify-assets/);

  assert.match(checklist, /npm run release:publish -- --version 4\.10\.9/);
...
test("Windows README documents zip installation and MCP client command paths", () => {
  ...
  assert.match(windowsReadme, /ace-mcp-v4\.10\.9-win-x64\.zip/);
...
test("release checklist records the v4.10.9 verification gates", () => {
  const checklist = readFileSync(path.join(rootDir, "docs/release-checklist.md"), "utf8");

  assert.match(checklist, /v4\.10\.9/);
  ...
  assert.match(checklist, /git tag -a v4\.10\.9/);
});

test("v4.10.9 release docs describe summary-assisted project routing", () => {
  const readme = readFileSync(path.join(rootDir, "README.md"), "utf8");
  const changelog = readFileSync(path.join(rootDir, "CHANGELOG.md"), "utf8");
  const roadmap = readFileSync(path.join(rootDir, "ROADMAP.md"), "utf8");

  assert.match(readme, /当前版本：`v4\.10\.9`/);
  assert.match(readme, /### v4\.10\.9（当前版本）/);
