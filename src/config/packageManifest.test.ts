...
test("package manifest is ready for npm and tgz global installation", () => {
  ...
  assert.equal(pkg.version, "4.10.10");
  assert.equal(lock.version, pkg.version);
  assert.equal(lock.packages?.[""]?.version, pkg.version);
  assert.match(versionTs, /APP_VERSION\s*=\s*"4\.10\.10"/);
  ...
  assert.equal(
    pkg.scripts["release:benchmark"],
    "node scripts/benchmark-search.mjs --smoke --release-gate --iterations 2 --search-p95-threshold-ms 5000 --search-p99-threshold-ms 5000",
  );
...
test("Web runtime starts and stops automatic index updates", () => {
  ...
  assert.match(entrypoint, /shutdown[\s\S]*searchService\.close\(\)/);
});
...
test("macOS quick install script and docs are packaged for one-command setup", () => {
  ...
  assert.match(installScript, /ACE_MCP_VERSION="\$\{ACE_MCP_VERSION:-4\.10\.10\}"/);
  assert.match(readme, /bash -c "\$\(curl -fsSL https:\/\/gitee\.com\/AndrewFengCode\/ace-mcp\/raw\/v4\.10\.10\/scripts\/install-macos\.sh\)"/);
  ...
  assert.match(readme, /ACE_MCP_VERSION=4\.10\.10/);
...
test("release asset verifier documents Gitee tag and downloadable artifacts", () => {
  ...
  assert.match(readme, /npm run release:verify-assets -- --version 4\.10\.10/);
  assert.match(readme, /raw\/v4\.10\.10\/scripts\/install-macos\.sh/);
  assert.doesNotMatch(readme, /raw\/master\/scripts\/install-macos\.sh/);

  assert.match(checklist, /npm run release:verify-assets -- --version 4\.10\.10/);
  assert.match(checklist, /ace-mcp-4\.10\.10\.tgz/);
  assert.match(checklist, /ace-mcp-v4\.10\.10-win-x64\.zip/);
  assert.match(checklist, /tar -tf ace-mcp-4\.10\.10\.tgz > \/tmp\/ace-mcp-tgz-files\.txt/);
  ...
  assert.match(checklist, /unzip -Z1 release\/ace-mcp-v4\.10\.10-win-x64\.zip > \/tmp\/ace-mcp-win-files\.txt/);
  assert.match(checklist, /rg -Fx "ace-mcp-v4\.10\.10-win-x64\/dist\/web\/static\/js\/app\.js" \/tmp\/ace-mcp-win-files\.txt/);
  assert.match(checklist, /rg -Fx "ace-mcp-v4\.10\.10-win-x64\/dist\/web\/static\/css\/main\.css" \/tmp\/ace-mcp-win-files\.txt/);
});
...
test("Gitee release publisher documents token-based automated release upload", () => {
  ...
  assert.match(readme, /npm run release:publish -- --version 4\.10\.10/);
  assert.match(readme, /GITEE_TOKEN/);
  assert.match(readme, /release:verify-assets/);

  assert.match(checklist, /npm run release:publish -- --version 4\.10\.10/);
...
test("Windows README documents zip installation and MCP client command paths", () => {
  ...
  assert.match(windowsReadme, /ace-mcp-v4\.10\.10-win-x64\.zip/);
...
test("release checklist records the v4.10.10 verification gates", () => {
  const checklist = readFileSync(path.join(rootDir, "docs/release-checklist.md"), "utf8");

  assert.match(checklist, /v4\.10\.10/);
  ...
  assert.match(checklist, /git tag -a v4\.10\.10/);
});

test("v4.10.10 release docs describe summary symbol enumeration and forced refresh", () => {
  const readme = readFileSync(path.join(rootDir, "README.md"), "utf8");
  const changelog = readFileSync(path.join(rootDir, "CHANGELOG.md"), "utf8");
  const roadmap = readFileSync(path.join(rootDir, "ROADMAP.md"), "utf8");

  assert.match(readme, /当前版本：`v4\.10\.10`/);
  assert.match(readme, /### v4\.10\.10（当前版本）/);
  assert.match(readme, /真实关键符号枚举/);
  assert.match(readme, /force: true/);
  assert.match(readme, /--force-summary/);
  assert.match(readme, /### v4\.10\.9/);
  assert.match(readme, /项目摘要.*附加.*路由证据/s);
  assert.match(readme, /architecture.*模块描述.*keySymbols/s);
  assert.match(readme, /mtime.*缓存/s);
  assert.match(readme, /摘要.*缺失.*损坏.*回退/s);
  assert.match(readme, /sandbox_workspace_write.*writable_roots/s);
  assert.match(readme, /### v4\.10\.8/);
  assert.match(readme, /有限.*search worker pool/s);
  assert.match(readme, /8\/16\/32.*并发/s);
  assert.match(readme, /watcher.*periodic-only/s);
  assert.match(readme, /maintenance lease.*续租/s);
  assert.match(readme, /### v4\.10\.7/);
  assert.match(readme, /maintenance lease.*stdio MCP freshness/s);
  assert.match(changelog, /## \[4\.10\.10\] - 2026-08-28/);
  assert.match(changelog, /findDefinitions\(projectId, "\*"\)/);
  assert.match(changelog, /强制模式.*跳过.*缓存/s);
  assert.match(changelog, /## \[4\.10\.9\] - 2026-08-27/);
  assert.match(changelog, /项目摘要.*路由证据/s);
  assert.match(changelog, /mtime.*缓存/s);
  assert.match(changelog, /摘要.*缺失.*损坏.*回退/s);
  assert.match(changelog, /## \[4\.10\.8\] - 2026-08-14/);
  assert.match(changelog, /8\/16\/32.*并发/s);
  assert.match(changelog, /periodic-only/);
  assert.match(changelog, /主动续租/);
  ...
  assert.match(roadmap, /摘要关键符号与强制刷新.*v4\.10\.10/);
  assert.match(roadmap, /摘要辅助项目路由.*v4\.10\.9/);
  assert.match(roadmap, /Codex 沙箱初始化闭环.*v4\.10\.7/);
  assert.match(roadmap, /跨进程索引写入协调.*v4\.10\.6/);
});

test("v4.10.5 release docs preserve stable Git dirty reconciliation history", () => {
  const readme = readFileSync(path.join(rootDir, "README.md"), "utf8");
  const changelog = readFileSync(path.join(rootDir, "CHANGELOG.md"), "utf8");
  const roadmap = readFileSync(path.join(rootDir, "ROADMAP.md"), "utf8");

  assert.match(readme, /### v4\.10\.5/);
