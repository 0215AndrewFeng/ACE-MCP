...
- **QA 下游实现自动发现**：ASK 管线在搜索到方法定义后，自动通过索引调用图（`findCallees`）提取被调用的方法名，执行补充搜索并将下游 Service/工具类实现注入 LLM 上下文。解决了此前只找到 Controller 方法定义、看不到核心业务实现的问题
- **源码级方法调用提取**：新增 `extractMethodCallsFromSnippet`，从源码片段中提取方法调用名（而非仅定义），作为调用图索引的补充回退
