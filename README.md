...
从 v4.10.5 Gitee Release 下载 `ace-mcp-4.10.5.tgz` 安装：

```bash
npm install -g ./ace-mcp-4.10.5.tgz
ace-mcp-web
```
...
```bash
bash -c "$(curl -fsSL https://gitee.com/AndrewFengCode/ace-mcp/raw/v4.10.5/scripts/install-macos.sh)"
```

安装指定版本：

```bash
ACE_MCP_VERSION=4.10.5 bash -c "$(curl -fsSL https://gitee.com/AndrewFengCode/ace-mcp/raw/v4.10.5/scripts/install-macos.sh)"
```
...
```bash
curl -LO https://gitee.com/AndrewFengCode/ace-mcp/releases/download/v4.10.5/ace-mcp-4.10.5.tgz
npm install -g ./ace-mcp-4.10.5.tgz
ace-mcp --version
ace-mcp-web
```
...
v4.10.5 Windows ZIP 尚待在 Windows x64 + Node.js 22 主机上构建和验证。产物补齐并发布后，下载 `ace-mcp-v4.10.5-win-x64.zip`，解压后双击 `start-web.cmd`，然后访问 <http://127.0.0.1:8787/>：
...
```json
{
  "mcpServers": {
    "ace-mcp": {
      "command": "C:\\Tools\\ace-mcp-v4.10.5-win-x64\\ace-mcp.cmd"
    }
  }
}
```
...
`release:win` 必须在 Windows x64 + Node.js 22 上运行。它会把当前 `node.exe`、裁剪后的生产依赖和已加载验证的 `better-sqlite3` 原生二进制一起打入 ZIP；当前 Darwin 主机不能完成或替代这项验证，v4.10.5 Windows ZIP 尚未补齐。
...
| `--warm` | 在 readiness 前完成暖机，再开放 MCP/Web 服务，消除首次查询延迟 |
...
- **自动项目路由**：新增一次有界的全局候选召回，根据问题关键词返回 `single`、`multiple` 或 `abstain`，并附带置信度、命中词和证据文件
