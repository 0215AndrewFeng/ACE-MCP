...
41. **QA 性能（总耗时近 2 分钟）**：实测单次问答 ~109s。瓶颈：① queryExpansion 标称 8s 超时但实测 55s 未真正中断（慢端点对 abort 不敏感）；② reranker 默认开启额外 10s；③ 最终回答受慢 LLM 端点拖累 41s。需让超时真正生效（兜底/竞速）、评估默认关闭 reranker、并行化可选增强
