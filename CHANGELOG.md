...
- 自动项目路由复用已有 `.ace-mcp/summaries/project-summary.json`，把项目目录名、`architecture`、模块路径、模块描述和 `keySymbols` 转成有界的项目级路由词集合；自然语言业务问题即使没有同名代码文本，也可通过项目摘要形成候选。
- 摘要命中以 `summary` 来源作为附加路由证据，权重介于 lexical 与 symbol 之间，并继续与代码全文、符号、关键词覆盖、项目名 ownership 和 single/multiple/abstain 阈值统一评分；低覆盖摘要不会单独选中项目。
- 每个项目的摘要路由词按文件 `mtime` 和大小缓存在内存中，仅在摘要变化后重新读取；摘要缺失、超限、损坏或字段无效时静默回退到现有代码与符号路由，不在查询过程中调用 LLM 或自动生成摘要。

### Validation

- 路由回归覆盖摘要单独召回、缓存刷新、仓库名、完整关键符号、损坏摘要回退和低覆盖 abstain，并保留原有父子项目、混合 ASCII/CJK、公平召回、重复证据衰减和弱信号测试。
...
- `includeContextLines` 共享上限从 50 提升到 200，MCP schema 与 Web 请求解析继续使用同一常量，Web 传入超大值会钳制到 200。
- Web 搜索选项的“上下文行数”新增“最大”按钮，文件片段查看的行号范围新增“最大”按钮（起始行设为 1、结束行设为 999999，由后端按实际 EOF 钳制）。
- Web 智能问答高级选项新增 `maxContextTokens` 输入，默认 48000、最大 200000，并在 SSE 请求中传给后端，配合“参考代码数量”最大按钮减少大接口漏代码风险。
- 新增请求校验、共享 schema 与静态前端契约测试，覆盖更大的上下文窗口和最大快捷按钮。
